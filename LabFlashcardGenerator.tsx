...
import { 
  ...
  RotateCw, AlertCircle, Bookmark, Tag, CheckCircle2, GraduationCap, Lightbulb
} from 'lucide-react';
import { gemini } from './geminiService';
import { Flashcard, View } from './types';

/* Changed to named export to resolve resolution issues in AILabPage.tsx */
export const LabFlashcardGenerator = ({ onNavigate }: { onNavigate: (view: View) => void }) => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-8 rounded-[40px] border border-primary/5 shadow-sm flex flex-col md:flex-row items-center gap-8">
        <div className="w-16 h-16 bg-orange-500/10 rounded-2xl flex items-center justify-center shrink-0 text-orange-500">
          <Bookmark size={24} />
        </div>
        <div className="flex-1 text-center md:text-left">
          <h3 className="text-xl font-poppins font-bold text-primary mb-2 flex items-center justify-center md:justify-start gap-2">
            <span>Module Briefing: Flashcard Generator</span>
            <div className="px-2 py-0.5 bg-orange-100 rounded text-[8px] font-black text-orange-500 uppercase tracking-tighter">Cognitive Aid</div>
          </h3>
          <p className="text-sm text-primary/60 font-medium leading-relaxed max-w-3xl">
            Converts study material into active-recall questions for better retention. This module isolates unique concepts and transforms them into interactive prompts with difficulty tagging.
          </p>
        </div>
        <div className="w-px h-12 bg-primary/5 hidden md:block" />
        <div className="flex items-center gap-3">
          <Lightbulb size={24} className="text-orange-500/30" />
        </div>
      </div>
